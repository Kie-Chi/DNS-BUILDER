...
from typing import Dict, Any, List, Optional, Tuple, TypeVar, Generic, Type, Set, ClassVar, TYPE_CHECKING
import logging
import json
import re
import hashlib
...
class Includer(ABC):
    ...
    def assemble(self) -> None:
        ...
        It should:
        For each ConfigFragment, for a generic DNS software
        ...
        To see what a repeatable fragment like, you can refer to the `Class SectionInfo`/`Class Section`
...
class BaseIncluder(Includer, ABC):
    """
    Generic Includer base class that handles configuration fragment assembly.

    Subclasses only need to implement:
    - inject: Block injection logic (BIND uses brace counting, Unbound returns False)

    The include_tpl is fetched from the Section class associated with this software.

    The assemble() method provides a unified workflow:
        For global fragments: append include directly
        For repeatable sections: create new block and append
        For non-repeatable sections: try to inject into existing block, or create new one
    """
    ...
    def assemble(self) -> None:
        ...
        for section in self.get_all_sections():
            for fragment in self.get_pendings(section):
                self._include_fragment(fragment)

    def _include_fragment(self, fragment: 'ConfigFragment') -> None:
        """Process a single fragment and include it appropriately."""
        section = fragment.section
        include_line = self._format_include(fragment.dst)

        if section == "global":
            self.fs.append_text(self.main.src, include_line)
            logger.debug(f"Included global fragment '{fragment.dst}'")
        elif self._should_inject(section):
            origin = self.fs.read_text(self.main.src)
            updated, success = self.inject(origin, section, [include_line])
            if success:
                self.fs.write_text(self.main.src, updated)
                logger.debug(f"Injected fragment '{fragment.dst}' into existing '{section}' block")
            else:
                block = self._create_block(section, include_line, fragment.params)
                self.fs.append_text(self.main.src, block)
    ...
    def _get_include_tpl(self) -> str:
        """Get the include template from the Section class."""
        if not self.software_type:
            return ""
        from .registry import section_registry
        section_cls = section_registry.section(self.software_type)
        if section_cls:
            return section_cls.include_tpl
        return ""

    def _format_include(self, path: str) -> str:
        """Generate include statement with comment header."""
        tpl = self._get_include_tpl()
        if not tpl:
            return f"\n# Auto Generated by DNS-Builder\n# Include: {path}\n"
        return f"\n# Auto Generated by DNS-Builder\n{tpl.format(path=path)}\n"

    def _create_block(self, section: str, content: str, params: Optional[Dict[str, str]] = None) -> str:
        """Create a new section block using Section template."""
        section_info = self.get_section_info(section)
        if section_info:
            formatted = section_info.format_content(content, **(params or {}))
            return f"\n# Auto Generated by DNS-Builder\n{formatted}\n"
        return f"\n# Auto Generated by DNS-Builder\n{content}\n"

    def _should_inject(self, section: str) -> bool:
        """
        Determine if content should be injected into existing block.

        Returns True if section is non-repeatable and has a block pattern.
        """
        if self.is_repeatable(section):
            return False
        section_info = self.get_section_info(section)
        return section_info is not None and section_info.block_pattern is not None

    @abstractmethod
    def inject(self, content: str, section: str, lines: List[str]) -> Tuple[str, bool]:
        """
        Inject content lines into an existing block.

        Args:
            content: The current configuration file content
            section: The section name to inject into
            lines: Lines to inject

        Returns:
            - updated_content: The modified content (unchanged if failed)
