...
class Builder:
    ...
    def _process_behavior(self, service_name: str, build_conf: Dict, image_obj: Image, contents_dir: str, main_conf_path: Optional[str], volumes: List[str]):
        ...
        for line in behavior_str.strip().split('\n'):
            ...
            if not target_ip:
                raise ValueError(f"Behavior in '{service_name}' references undefined service '{behavior_obj.target_name}'.")
        ...
        # --- Generate software type ---
        generated_zones_content = ""
        if image_obj.software == "unbound":
            # Unbound, create a structured file with server: and top-level sections
            if all_config_lines['server']:
            ...
            if all_config_lines['toplevel']:
                generated_zones_content += "\n" # Separator
                generated_zones_content += "\n\n".join(all_config_lines['toplevel'])

        else: 
            # BIND, just join the lines
            all_lines = all_config_lines['toplevel'] + all_config_lines.get('server', [])
            generated_zones_content = "\n".join(all_lines)

        if not generated_zones_content.strip():
            return # Don't create empty files or includes

        # Write the final generated config file
